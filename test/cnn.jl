...
function convolution(x, w; stride = 1, pad = 0)
    ...
    @tullio y[wo, ho, co, n] := x[(wo-1)*$stride+kw, (ho-1)*$stride+kh, ci, n] * w[Ww - kw + 1, Hw - kh + 1, ci, co] (wo in 1:Wo, ho in 1:Ho, kw in 1:Ww, kh in 1:Hw)
end
...
@testset "convolution" begin
    @testset "stride=1, pad=0" begin
        ...
        b_manual = @benchmark convolution($x, $w)
        b_nnlib = @benchmark NNlib.conv($x, $w)
    ...
    @testset "stride=2, pad=1" begin
        ...
        b_manual = @benchmark convolution($x, $w; stride=$stride, pad=$pad)
        b_nnlib = @benchmark NNlib.conv($x, $w; stride=$stride, pad=$pad)
